...
gpdata.dtw.none<- get_gpdata(msr.dtw.none, res[[6]]$cvparam.new$wrap.FDboost.fpco.dtw, subname = c("pve","add"))
...
plot8 <- ggplot(gpdata.fpc, aes(x = pve, y = msr, fill = pve)) + geom_boxplot(fill = "lightblue") + 
  stat_summary(fun.y=mean, geom="point", shape=23, size=4) + 
  scale_fill_brewer(palette="Blues") + ylim(0,0.45) + 
