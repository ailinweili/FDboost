...
plot1 <- ggplot(gpdata.intercept, aes(x = type, y = mse, fill = type)) + geom_boxplot(fill = "lightblue") + 
  ...
  scale_fill_brewer(palette="Blues") + ylim(0,65) + 
...
gpdata.gam <- get_gpdata(mse.gam, res$wrap.gam.fpco$cvparam.new$wrap.gam.fpco, subname = c("p", "k","add"))
plot11 <- ggplot(gpdata.gam, aes(x = k , y = mse, fill = add)) + geom_boxplot() +
...
rownames(plotdata) <- c("FDb.fpco.mink", "FDb.fpco.ela","FDb.fpco.corr",
                        "FDb.fpco.dtw","FDb.fpc","FDb.bsig","pfr.af", "gam.fpco.mink")
